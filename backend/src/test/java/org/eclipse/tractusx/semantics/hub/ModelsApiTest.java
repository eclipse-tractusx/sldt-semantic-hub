...
public class ModelsApiTest extends AbstractModelsApiTest{
  ...
  public void testAasxEndpointExpectSuccess() throws Exception {
   ...
   public void testInvalidSAMMModel() throws Exception {
       String urnPrefix = "urn:samm:org.eclipse.tractusx.invalid.model.save:2.0.0#";
       String model = TestUtils.loadModelFromResources(TestUtils.VALIDATION_FAILED_SAMM ).replace("{{URN_PREFIX}}", urnPrefix);
      ...
      mvc.perform( MockMvcRequestBuilders.post( "/api/v1/models" )
            ...
            .andExpect( status().isBadRequest() )
            .andExpect( jsonPath( "$.error.message", containsString("Validation failed" ) ) )
            .andExpect( jsonPath( "$.error.details.ERR_INVALID_PROPERTY", containsString("':ChildData' properties list must be a property - either directly or via a reference to a property with an attribute" ) ) );
   }
